(function(){
  ...
  function renderBudgetJourneyCard(initiatives){
    ...
    return `<article class="budget-journey-card card has-tooltip" data-tooltip="Exact recorded portfolio amounts for each budget stage. Missing entries are reported as missing and are never converted to RM 0."><div class="budget-journey-header"><div><span class="eyebrow">Budget journey</span><h2>Estimate to approval</h2></div><div class="budget-journey-saving ${savings<0?'negative':''}"><small>${savings<0?'Budget increase':'Estimate-to-approval reduction'}</small><strong>${savingsLabel}</strong><span>${escapeHtml(savingsMeta)}</span></div></div><div class="budget-journey-stages">${stages.map((r,idx)=>`<div class="budget-stage ${stageClass[idx]} has-tooltip" data-tooltip="${escapeAttr(r.label+': '+(r.coverage?money(r.value):'Not recorded')+'. '+stageHelp[idx]+' Coverage: '+r.coverage+' of '+r.total+' initiative cycles; '+(r.total-r.coverage)+' missing.')}"><span>${escapeHtml(r.label)}</span><strong>${r.coverage?money(r.value):'Not recorded'}</strong><small class="budget-stage-help">${stageShort[idx]}</small><small class="budget-stage-coverage">${r.coverage} of ${r.total} recorded${r.total-r.coverage?` · ${r.total-r.coverage} missing`:''}</small></div>`).join('')}</div><div class="budget-journey-footer"><span><b>${comparable.length}</b> Initial vs Approved comparable</span><span><b>${completeJourneyCount}</b> complete four-stage</span><span><b>${missing}</b> missing stage values</span></div></article>`;
  }
