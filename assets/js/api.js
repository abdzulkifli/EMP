(function(){
  ...
  async function loadData(user){
    ...
    var results = await Promise.all([
      ...
      optionalRequestResult('/rest/v1/audit_logs?select=id,created_at,user_id,action,entity_type,entity_id,new_values,old_values&order=created_at.desc&limit=250')
    ...
    var actor = (results[6] || []).find(function(user) {
    ...
    if (row.new_values) {
    try {
      details = typeof row.new_values === 'string'
        ? row.new_values
